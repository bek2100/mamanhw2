...
void* balances(int ID, int ANumber) {
    ...
    sprintf(cmd, "SELECT SUM(diff) FROM (SELECT WID, WAmount, WCommission, WTime, 0 AS type, WCommission + WAmount AS diff FROM Withdrawal WHERE ANumber = %d "" UNION ALL "" SELECT TID, TAmount, TCommission, TTime,  0 AS type, TCommission + TAmount AS diff FROM Transfer Where ANumberF = %d "" UNION ALL "" SELECT TID, TAmount, TCommission, TTime,  1 AS type, 0-TAmount AS diff FROM Transfer Where ANumberT = %d) AS A ORDER BY TID", ANumber, ANumber, ANumber);
