...
void* associates(int ID) {
    ...
    sprintf(cmd, "SELECT IDF FROM Transfer WHERE IDT = (SELECT IDT FROM Transfer WHERE IDF =%d OR IDT =%d) OR IDT = %d "" UNION "" SELECT IDT FROM Transfer WHERE IDF = (SELECT IDF FROM Transfer WHERE IDT =%d OR IDF =%d) OR IDF = %d "" ORDER BY IDF", ID, ID, ID, ID, ID, ID);
