...
void* associates(int ID) {
    ...
    sprintf(cmd, "SELECT DISTINCT T.IDF FROM Transfer T WHERE EXISTS(SELECT T1.IDF FROM (SELECT IDF FROM Transfer WHERE IDT=%d OR IDF=%d UNION SELECT IDT FROM Transfer WHERE IDF=%d OR IDT=%d) T1 WHERE T.IDT=T1.IDF OR T.IDF=T1.IDF) ORDER BY T.IDF", ID, ID,ID, ID);
