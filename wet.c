...
void* associates(int ID) {
    ...
    sprintf(cmd, "SELECT DISTINCT T.IDF FROM Transfer T WHERE EXISTS(SELECT * FROM (SELECT IDF FROM Transfer WHERE IDT=%d UNION SELECT IDT FROM Transfer WHERE IDF=%d) T1 WHERE T.IDT=T1.IDF OR T.IDF=T1.IDF) OR T.IDF=%d OR T.IDT=%d UNION SELECT DISTINCT T.IDT FROM Transfer T WHERE EXISTS(SELECT * FROM (SELECT IDF FROM Transfer WHERE IDT=%d UNION SELECT IDT FROM Transfer WHERE IDF=%d) T1 WHERE T.IDT=T1.IDF OR T.IDF=T1.IDF) OR T.IDF=%d OR T.IDT=%d ORDER BY T.IDF", ID, ID,ID, ID, ID, ID, ID,ID, ID, ID);
...
void* moneyLaundering() {
   ...
   while (num_id){
    PQclear(res);
       ...
       sprintf(cmd, "INSERT INTO money (TID, IDF, IDT, TAmount) SELECT DISTINCT T1.TID, T.IDF, T1.IDT, T1.TAmount FROM money T INNER JOIN money T1 ON T.IDT=T1.IDF AND T.TAmount>=T1.TAmount AND T.TID<T1.TID AND NOT EXISTS(SELECT * FROM money M WHERE T1.TID=M.TID AND T.IDF=M.IDF AND T1.IDT=M.IDT AND T1.TAmount=M.TAmount)");
