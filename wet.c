...
void* balances(int ID, int ANumber) {
    ...
    sprintf(cmd, "SELECT WID, WAmount, WCommission, WTime,  0 AS type, WCommission + WAmount AS diff FROM Withdrawal WHERE ANumberF = %d "" UNION ALL "" SELECT TID, TAmount, Tcomission, TTime,  0 AS type, WCommission + WAmount AS diff FROM Transfer Where ANumberT = %d "" UNION ALL "" SELECT TID, TAmount, Tcomission, TTime,  1 AS type, WComission - WAmount AS diff FROM Transfer Where ANumberT = %d ""ORDER BY WID ", ANumber, ANumber, ANumber);
