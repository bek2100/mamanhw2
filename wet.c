...
void* moneyLaundering() {
   ...
   int i=0;
    ...
    sprintf(cmd, "CREATE TABLE money AS SELECT TID,TAmount,IDF,IDT,TAmount path FROM Transfer; SELECT * FROM money");
    res = PQexec(conn, cmd);
    if(!res || PQresultStatus(res) != PGRES_TUPLES_OK) { fprintf(stderr, "Error executing query: %s\n", PQresultErrorMessage(res)); return NULL; }
   ...
   while (num_id){
    PQclear(res);
       ...
       sprintf(cmd, "INSERT INTO money (TID, TAmount, IDF, IDT, path) SELECT T1.TID, T1.TAmount, T.IDF, T1.IDT, CONCAT(T.TAmount,T1.TAmount) FROM money T INNER JOIN money T1 ON T.IDT=T1.IDF AND T1.TAmount<=T.TAmount AND T.TID<T1.TID AND NOT EXISTS(SELECT * FROM money M WHERE T1.TID=M.TID AND T.IDF=M.IDF AND T1.IDT=M.IDT AND T1.TAmount=M.TAmount);");
