...
void* balances(int ID, int ANumber) {
    ...
    sprintf(cmd, "SELECT WID, WAmount, WCommission, WTime, 0 AS type, WCommission + WAmount AS diff FROM Withdrawal WHERE ANumber = %d "" UNION ALL "" SELECT TID, TAmount, TCommission, TTime,  0 AS type, TCommission + TAmount AS diff FROM Transfer Where ANumberF = %d "" UNION ALL "" SELECT TID, TAmount, TCommission, TTime,  1 AS type,  - TAmount AS diff FROM Transfer Where ANumberT = %d "" ORDER BY WTime ORDER BY WID", ANumber, ANumber, ANumber);
...
void* associates(int ID) {
    ...
    sprintf(cmd, "SELECT DISTINCT A.IDF FROM (SELECT DISTINCT T.IDF FROM Transfer T WHERE EXISTS(SELECT * FROM (SELECT IDF FROM Transfer WHERE IDT=%d UNION SELECT IDT FROM Transfer WHERE IDF=%d) T1 WHERE T.IDT=T1.IDF OR T.IDF=T1.IDF) UNION SELECT DISTINCT T.IDT FROM Transfer T WHERE EXISTS(SELECT * FROM (SELECT IDF FROM Transfer WHERE IDT=%d UNION SELECT IDT FROM Transfer WHERE IDF=%d) T1 WHERE T.IDT=T1.IDF OR T.IDF=T1.IDF)) AS A ORDER BY A.IDF", ID, ID,ID, ID, ID);
